...
def seed_progress(current, total, max_poll_count, seed_start, force_newline=False):
    ...
    progress_done = current >= total or (seeding_yaml["move_on_if_server_dead"] and max_poll_count > 9 and diff >= thresh_diff) or force_newline
...
try:
    ...
    stop_seeding_datetime = today.replace(hour=stop_seeding_time.hour, minute=stop_seeding_time.minute, second=0, microsecond=0)
    ...
    def perpetual_seeding_search():
        ...
        for server_addr in gs.query_master(r'\appid\686810\password\0\full\1\empty\1', max_servers=500):
            try:
                ...
                if not info.password_protected and info.player_count < seeding_yaml["seeded_player_limit"] and info.player_count >= seeding_yaml["perpetual_min_players"] and info.max_players == 100 and "CN" not in info.server_name:
    ...
    if len(valid_servers) > 0 or seeding_yaml["perpetual_seed_from_steam"]:
        ...
        while True:
            ...
            if dt.today() >= stop_seeding_datetime:
                print()
                print()
                print(f'{colors.fg.yellow}Seeding stop time reached [ {stop_seeding_datetime} ]. Closing game.{colors.reset}')
            ...
            try:
                ...
                if seeding_yaml["move_on_if_kicked"] and tried_connect and poll_count*seeding_yaml["server_query_rate"] > 60 and players != None:
                    name_present = False
                    names = []
                    ...
                    if not name_present:
                        seed_progress(info.player_count, seeding_yaml["seeded_player_limit"], max_poll_count, seed_start, True)
                        
                        names.sort()
                        print(f'{colors.fg.red}{seeding_yaml["player_name"]} is no longer in the player list. Idle kicked?{colors.reset}')
                ...
                if seeding_yaml["move_on_if_server_dead"] and max_poll_count >= seeding_yaml["perpetual_min_players"] and info.player_count <= max_poll_count / 2:
                ...
                if seeding_yaml["perpetual_dead_if_empty"] and perpetual_mode and info.player_count < seeding_yaml["perpetual_min_players"]-1:
                    seed_progress(info.player_count, seeding_yaml["seeded_player_limit"], max_poll_count, seed_start, True)
